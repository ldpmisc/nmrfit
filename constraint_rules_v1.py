...
from typing import List, Optional, Dict, Any, TYPE_CHECKING, Self, Tuple, Iterable, Set, Callable, Mapping
...
# --- J-COUPLING HELPERS (Hz) ---
def resolve_J_from_registry(
    *,
    J_name: str,
    Jseed_registry: Optional[Mapping[str, Mapping[str, Any]]] = None,
    default_J_seed_Hz: float = 10.0,
    default_J_lo_Hz: Optional[float] = None,
    default_J_hi_Hz: Optional[float] = None,
    default_fixed: bool = False,
) -> Tuple[float, ParamBounds, bool]:
    """
    Resolve a named scalar-coupling parameter from a registry.
    ...
    Returns (J_seed_Hz, J_bounds_Hz, fixed).
    """
    name = (J_name or "").strip()
    if not name:
        raise ValueError("Empty J_name.")

    rec = (Jseed_registry or {}).get(name)
    if rec is None:
        log.warning("J_name '%s' missing in J registry; using defaults.", name)
        return (
            float(default_J_seed_Hz),
            ParamBounds(lo=default_J_lo_Hz, hi=default_J_hi_Hz),
            bool(default_fixed),
        )

    fixed = bool(rec.get("fixed", default_fixed))
    J_seed = rec.get("J_seed_Hz", rec.get("J_seed", default_J_seed_Hz))
    J_lo = rec.get("J_lo_Hz", default_J_lo_Hz)
    J_hi = rec.get("J_hi_Hz", default_J_hi_Hz)

    J_seed = float(J_seed if J_seed is not None else default_J_seed_Hz)
    J_lo = None if J_lo is None else float(J_lo)
    J_hi = None if J_hi is None else float(J_hi)

    if not np.isfinite(J_seed):
        raise ValueError(f"J_seed_Hz must be finite for '{name}', got {J_seed}.")
    if J_lo is not None and not np.isfinite(J_lo):
        raise ValueError(f"J_lo_Hz must be finite for '{name}', got {J_lo}.")
    if J_hi is not None and not np.isfinite(J_hi):
        raise ValueError(f"J_hi_Hz must be finite for '{name}', got {J_hi}.")
    if (J_lo is not None) and (J_hi is not None) and (J_lo > J_hi):
        raise ValueError(f"Invalid J bounds for '{name}': lo({J_lo}) > hi({J_hi}).")

    return J_seed, ParamBounds(lo=J_lo, hi=J_hi), fixed


def ensure_J_param_in_lmfit(
    *,
    params,
    J_name: str,
    Jseed_registry: Optional[Mapping[str, Mapping[str, Any]]] = None,
    default_Jmin: float = -1e5,
    default_Jmax: float = 1e5,
) -> str:
    """Create or update shared lmfit parameter J__{J_name}, in Hz."""
    name = (J_name or "").strip()
    if not name:
        raise ValueError("Empty J_name.")

    J_key = f"J__{name}"
    J_seed, J_bounds, fixed = resolve_J_from_registry(
        J_name=name,
        Jseed_registry=Jseed_registry or {},
    )
    Jmin = default_Jmin if J_bounds.lo is None else float(J_bounds.lo)
    Jmax = default_Jmax if J_bounds.hi is None else float(J_bounds.hi)
    if Jmin > Jmax:
        Jmin, Jmax = Jmax, Jmin

    if J_key not in params:
        params.add(J_key, value=float(J_seed), min=float(Jmin), max=float(Jmax), vary=(not fixed))
    else:
        pJ = params[J_key]
        pJ.min = float(Jmin)
        pJ.max = float(Jmax)
...
class ConstraintType(Enum):
    ...
    LINEAR = "LINEAR"
    RELAX_DECAY = "RELAX DECAY"
    RELAX_GROWTH = "RELAX GROWTH"
    J_COUPLING = "J COUPLING"
    J_MODULATION = "J MODULATION"
...
def resolve_constraint_type(type_txt: str) -> "ConstraintType":
    ...
    if t in ("RELAX_DECAY", "RELAX DECAY", "RELAX", "DECAY", "RELAX EXP", "RELAX_EXP" ):
        return ConstraintType.RELAX_DECAY
    if t in ("RELAX_GROWTH", "RELAX GROWTH", "GROWTH", "EXP_GROWTH"):
        return ConstraintType.RELAX_GROWTH
    if t in ("J_COUPLING", "J COUPLING", "J", "JCOUPLING", "JC"):
        return ConstraintType.J_COUPLING
    if t in ("J_MODULATION", "J MODULATION", "J MOD", "JMOD", "J_ECHO", "J ECHO", "ECHO MOD", "ECHO_MOD"):
        return ConstraintType.J_MODULATION
...
class RelaxExpConstraintBase(ConstraintRule, ABC):
    """
    ...
    @abstractmethod
    def _factor_value(self, t: float, T: float) -> float:
        """Compute the exponential factor value for this constraint type."""
        raise NotImplementedError
    
    @abstractmethod
    def _factor_expr(self, t_term: str, T_term: str) -> str:
        """Return the exponential factor expression string for this constraint type."""
        raise NotImplementedError
    
    @staticmethod
    def _canonical_rhs(drv: str, A: float, C: float, factor_expr: str) -> str:
        """Construct the canonical right-hand side expression using the factor."""
        base = f"{drv}*{factor_expr}"
        if A != 1.0 and C != 0.0:
            base = f"{A}*{base}+{C}"
        elif A != 1.0:
            base = f"{A}*{base}"
    ...
    def parse_UItext(cls, UI_txt: str) -> Dict:
        ...
        # KV-style exp(...)
        if low.startswith("exp(") and "=" in low:
    ...
    def to_display_expr(self) -> str:
        ...
        expr_term = self._factor_expr(t_term, T_term) #either exp(-t/T) or 1 - exp(-t/T)
        return self._canonical_rhs(drv, A, C, expr_term)
    ...
    def interpret_ui(
        ...
        *,
        ...
        if driver_pref is None:
            raise ValueError(f"{cls.__name__} requires a driver parameter.")
        ...
        if (T_number is not None) and (T_name is not None):
            raise ValueError(f"{cls.__name__} requires either numeric T or T_name, not both.")
        if (T_number is None) and (T_name is None):
            raise ValueError(f"{cls.__name__} requires either numeric T or T_name.")
    ...
    def create_rule(
        ...
        *,
        ...
        """Construct RelaxExpConstraint from fully-resolved domain parameters.
    ...
    def apply_to_lmfit(
            ...
            *,
            ...
            same_slice = int(self.driver_pref.slice_id) == int(target_pref.slice_id)
            type_txt = getattr(self, "constraint_type", None)
            type_txt = type_txt.name if type_txt is not None else self.__class__.__name__
            ...
            if same_slice:
                log.warning(
                    f"{type_txt} constraint on {target_key} is inherently time-dependent and require cross-slice driver but uses same-slice driver."
                    f"Consider using LINEAR constraint instead for intra-slice relationships."
                )              
                if drv_key in params:
                    if t_val is None:
                        raise ValueError(
                            f"{type_txt} constraint on {target_key}: no time value available"
                        )                                                   
                ...
                if not info or "value" not in info:
                    raise ValueError(
                        f"Missing driver value for internal {type_txt} constraint: {fmt_pref(self.driver_pref)}"
                    )               
                drv_val = float(info["value"])
                if t_val is None:
                    raise ValueError(
                ...
                if is_float(T_val) and T_val >= 0:
                    T_val = float(T_val)
                    exp_term = self._factor_value(t_val, T_val)
            ...
            if not allow_external:
                raise ValueError(
                    f"Cannot apply cross-slice {type_txt} constraint on {target_key} when allow_external=False"
                )
            ...
            if not vary:
                ...
                if self.T_name: # T_name -> convert T_name to k_name and inject k_name into parameter if not already present
                    if Tseed_registry is None:
                        raise ValueError(f"RELAX DECAY uses T_name='{self.T_name}' but tseed_registry was not provided.")
...
class RelaxDecayConstraint(RelaxExpConstraintBase):
    """
    Relaxation exponential constraint: target = driver * A * exp(-t / T) + C
    ...
    def _factor_expr(self, t_term: str, T_term: str) -> str:
        """Decay: exp(-t_term/T_term)"""
        return f"exp(-{t_term}/{T_term})"
    ...
    def parse_rhs(cls, UI_rhs_raw: str) -> Dict: #
        ...
        A, driver_txt = RelaxExpConstraintBase._split_left_mul(left)
...
class RelaxGrowthConstraint(RelaxExpConstraintBase):
    """
    ...
    def _factor_expr(self, t_term: str, T_term: str) -> str:
        """Growth: (1-exp(-t_term/T_term))"""
        return f"(1-exp(-{t_term}/{T_term}))"
...
class JCouplingConstraint(ConstraintRule):
    """
    First-order scalar-coupling position constraint:
        pos_target = pos_driver + sign * order * J * scale

    J is in Hz. Use scale=1 for Hz axes. Use scale=1/ref_MHz for ppm axes.
    """
    def __init__(self, *, target_pref: ParamRef, driver_pref: ParamRef,
                 J_number: Optional[float] = None, J_name: Optional[str] = None,
                 order: float = 1.0, sign: float = 1.0, scale: float = 1.0,
                 enabled: bool = True, expr_txt: str = ""):
        self.target_pref = target_pref
        self.driver_pref = driver_pref
        self.J_number = float(J_number) if J_number is not None else None
        self.J_name = (str(J_name).strip() if J_name is not None else None) or None
    ...
    @classmethod
    def parse_UItext(cls, UI_txt: str) -> Dict[str, Any]:
        kv = LinearConstraint.parse_kv(UI_txt)
        driver_txt = str(kv.get("driver", "")).strip()
        if not driver_txt:
            raise ValueError("JCouplingConstraint requires driver=<ParamRef>.")

        J_number = None
        J_name = kv.get("J_name", kv.get("j_name", None))
        J_raw = kv.get("J", kv.get("j", None))
        if J_raw is not None:
            if is_float(str(J_raw)):
                J_number = float(J_raw)
            else:
                J_name = str(J_raw).strip()
        if J_name is not None and str(J_name).strip() == "":
            J_name = None
        if (J_number is None) and (J_name is None):
            raise ValueError("JCouplingConstraint requires numeric J or J_name.")
        if (J_number is not None) and (J_name is not None):
            raise ValueError("Provide either numeric J or J_name, not both.")

        return {
            "driver_txt": driver_txt,
            "J_number": J_number,
            "J_name": str(J_name).strip() if J_name is not None else None,
            "order": float(kv.get("order", kv.get("n", 1.0))),
            "sign": float(kv.get("sign", 1.0)),
            "scale": float(kv.get("scale", 1.0)),
        }

    @classmethod
    def infer_driver_from_expr(cls, expr_txt: str) -> Optional[str]:
        return cls.parse_UItext(expr_txt).get("driver_txt")

    @classmethod
    def interpret_ui(cls, *, target_pref: ParamRef, driver_txt: str, expr_txt: str,
                     enabled: bool, ctx: "ParseContext") -> Optional["JCouplingConstraint"]:
        out = cls.parse_UItext(expr_txt)
        driver_pref = None
        if driver_txt and str(driver_txt).strip():
            try:
                ...
                driver_pref = None
        if driver_pref is None:
            driver_pref = txt_to_pref(out["driver_txt"])
        if str(target_pref.name).lower() != "pos" or str(driver_pref.name).lower() != "pos":
            raise ValueError("JCouplingConstraint should be used only for position parameters: *_pos.")
        return cls.create_rule(
            target_pref=target_pref,
            driver_pref=driver_pref,
            J_number=out.get("J_number"),
            J_name=out.get("J_name"),
            order=out.get("order", 1.0),
            sign=out.get("sign", 1.0),
    ...
    @classmethod
    def create_rule(cls, *, target_pref: ParamRef, driver_pref: ParamRef,
                    J_number: Optional[float] = None, J_name: Optional[str] = None,
                    order: float = 1.0, sign: float = 1.0, scale: float = 1.0,
                    enabled: bool = True, expr_txt: str = "") -> "JCouplingConstraint":
        return cls(target_pref=target_pref, driver_pref=driver_pref, J_number=J_number,
                   J_name=J_name, order=order, sign=sign, scale=scale,
                   enabled=enabled, expr_txt=expr_txt)

    def _j_term_for_expr(self) -> str:
        if self.J_name:
            return f"J__{self.J_name}"
        if self.J_number is not None:
            return str(float(self.J_number))
        raise ValueError("JCouplingConstraint has neither J_number nor J_name.")

    def to_display_expr(self) -> str:
        s = (self.expr_txt or "").strip()
        if s:
            return s
        J_term = self.J_name if self.J_name else self.J_number
        return f"driver={fmt_pref(self.driver_pref)}, J={J_term}, order={self.order}, sign={self.sign}, scale={self.scale}"

    def to_lmfit_expr(self) -> str:
        offset = float(self.sign) * float(self.order) * float(self.scale)
        return f"{fmt_pref(self.driver_pref)}+({offset})*({self._j_term_for_expr()})"

    def validate(self, target_peak, target_pref: ParamRef, driver_peak, driver_pref, slice_states) -> List[str]:
        errors = []
        if driver_peak is None or driver_pref is None:
            errors.append(f"J-coupling constraint on {fmt_pref(target_pref)}: driver peak not found")
        if str(target_pref.name).lower() != "pos" or str(self.driver_pref.name).lower() != "pos":
            errors.append(f"J-coupling constraint on {fmt_pref(target_pref)} must link position to position.")
        for label, val in (("J", self.J_number), ("order", self.order), ("sign", self.sign), ("scale", self.scale)):
            if val is not None and not np.isfinite(float(val)):
                errors.append(f"J-coupling constraint on {fmt_pref(target_pref)}: {label}={val} is not finite")
        return errors

    def apply_to_lmfit(self, *, params, registry, allow_external: bool = False, vary: bool = True,
                       Tseed_registry: Optional[Mapping[str, Mapping[str, Any]]] = None,
                       Jseed_registry: Optional[Mapping[str, Mapping[str, Any]]] = None) -> None:
        target_key = fmt_pref(self.target_pref)
        if target_key not in params:
            return
        if self.J_name:
            ensure_J_param_in_lmfit(params=params, J_name=self.J_name, Jseed_registry=Jseed_registry or Tseed_registry or {})
        drv_key = fmt_pref(self.driver_pref)
        same_slice = int(self.driver_pref.slice_id) == int(self.target_pref.slice_id)
        if same_slice and drv_key in params:
            params[target_key].set(expr=self.to_lmfit_expr(), vary=True)
            return
        if not allow_external:
            raise ValueError(f"Cannot apply cross-slice J-coupling on {target_key} when allow_external=False")
        if vary and drv_key in params:
            params[target_key].set(expr=self.to_lmfit_expr(), vary=True)
            return
        info = registry.get(self.driver_pref)
        if not info or "value" not in info:
            raise ValueError(f"Missing external driver value for {fmt_pref(self.driver_pref)}")
        if self.J_name:
            if f"J__{self.J_name}" in params:
                J_val = float(params[f"J__{self.J_name}"].value)
            else:
                J_val = resolve_J_from_registry(J_name=self.J_name, Jseed_registry=Jseed_registry or Tseed_registry or {})[0]
        else:
            J_val = float(self.J_number)
        val = float(info["value"]) + float(self.sign) * float(self.order) * J_val * float(self.scale)
        params[target_key].set(value=float(val), expr="", vary=False)
...
class JModulationConstraint(ConstraintRule):
    ...
    def __init__(self, *, target_pref: ParamRef, driver_pref: ParamRef,
                 J_number: Optional[float] = None, J_name: Optional[str] = None,
                 T2_number: Optional[float] = None, T2_name: Optional[str] = None,
                 A: float = 1.0, C: float = 0.0, time_s: Optional[float] = None,
                 enabled: bool = True, expr_txt: str = ""):
        self.target_pref = target_pref
        self.driver_pref = driver_pref
        self.J_number = float(J_number) if J_number is not None else None
        self.J_name = (str(J_name).strip() if J_name is not None else None) or None
        self.T2_number = float(T2_number) if T2_number is not None else None
        self.T2_name = (str(T2_name).strip() if T2_name is not None else None) or None
        self.A = float(A)
        self.C = float(C)
        self.time_s = float(time_s) if time_s is not None else None
        self.enabled = bool(enabled)
    ...
    @classmethod
    def parse_UItext(cls, UI_txt: str) -> Dict[str, Any]:
        """Parse: driver=s0_p0_amp, J=120, T2=0.05, A=1, C=0, time=0.001."""
        kv = LinearConstraint.parse_kv(UI_txt)
        driver_txt = str(kv.get("driver", "")).strip()
        if not driver_txt:
            raise ValueError("JModulationConstraint requires driver=<ParamRef>.")
        ...
        if J_name is not None and str(J_name).strip() == "":
            J_name = None
        if (J_number is None) and (J_name is None):
            raise ValueError("JModulationConstraint requires numeric J or J_name.")
        if (J_number is not None) and (J_name is not None):
            raise ValueError("Provide either numeric J or J_name, not both.")

        T2_number = None
        T2_name = kv.get("T2_name", kv.get("t2_name", None))
        T2_raw = kv.get("T2", kv.get("t2", None))
        if T2_raw is not None:
            if is_float(str(T2_raw)):
                T2_number = float(T2_raw)
            else:
                T2_name = str(T2_raw).strip()
        if T2_name is not None and str(T2_name).strip() == "":
            T2_name = None
        if (T2_number is None) and (T2_name is None):
            raise ValueError("JModulationConstraint requires numeric T2 or T2_name.")
        if (T2_number is not None) and (T2_name is not None):
            raise ValueError("Provide either numeric T2 or T2_name, not both.")
        if T2_number is not None and T2_number <= 0:
            raise ValueError(f"T2 must be > 0; got {T2_number}.")

        time_raw = kv.get("time", kv.get("time_s", kv.get("t", None)))
        time_s = float(time_raw) if time_raw is not None and str(time_raw).strip() != "" else None
        ...
        return {
            ...
            "J_name": str(J_name).strip() if J_name is not None else None,
            "T2_number": T2_number,
            "T2_name": str(T2_name).strip() if T2_name is not None else None,
            "A": float(kv.get("A", kv.get("a", 1.0))),
            "C": float(kv.get("C", kv.get("c", 0.0))),
            "time_s": time_s,
        }
    ...
    @classmethod
    def interpret_ui(cls, *, target_pref: ParamRef, driver_txt: str, expr_txt: str,
                     enabled: bool, ctx: "ParseContext") -> Optional["JModulationConstraint"]:
        out = cls.parse_UItext(expr_txt)
        driver_pref = None
        if driver_txt and str(driver_txt).strip():
            try:
        ...
        if driver_pref is None:
            driver_pref = txt_to_pref(out["driver_txt"])
        if str(target_pref.name).lower() != "amp" or str(driver_pref.name).lower() != "amp":
            raise ValueError("JModulationConstraint should be used only for amplitude parameters: *_amp.")

        time_s = out.get("time_s")
        if time_s is None:
            time_s = ctx.resolve_time_s(slice_id=target_pref.slice_id, t_override_s=None)
        if out.get("T2_name"):
            _ = ctx.get_T_record(out["T2_name"])
        ...
        return cls.create_rule(
            ...
            J_number=out.get("J_number"),
            J_name=out.get("J_name"),
            T2_number=out.get("T2_number"),
            T2_name=out.get("T2_name"),
            A=out.get("A", 1.0),
            C=out.get("C", 0.0),
            time_s=time_s,
            enabled=enabled,
    ...
    def create_rule(cls, *, target_pref: ParamRef, driver_pref: ParamRef,
                    ...
                    T2_number: Optional[float] = None, T2_name: Optional[str] = None,
                    A: float = 1.0, C: float = 0.0, time_s: Optional[float] = None,
                    enabled: bool = True, expr_txt: str = "") -> "JModulationConstraint":
        return cls(target_pref=target_pref, driver_pref=driver_pref,
                   J_number=J_number, J_name=J_name,
                   T2_number=T2_number, T2_name=T2_name,
                   A=A, C=C, time_s=time_s,
                   enabled=enabled, expr_txt=expr_txt)
    ...
    def _j_term_for_expr(self) -> str:
        ...
        if self.J_number is not None:
            return str(float(self.J_number))
        raise ValueError("JModulationConstraint has neither J_number nor J_name.")

    def _t2_term_for_expr(self) -> str:
        if self.T2_name:
            return f"k__{self.T2_name}"
        if self.T2_number is not None:
            return str(float(self.T2_number))
        raise ValueError("JModulationConstraint has neither T2_number nor T2_name.")

    def _factor_value(self, J_Hz: float, T2_s: float) -> float:
        if self.time_s is None:
    ...
    def to_display_expr(self) -> str:
        ...
        if s:
            return s
        J_term = self.J_name if self.J_name else self.J_number
        T2_term = self.T2_name if self.T2_name else self.T2_number
        return f"driver={fmt_pref(self.driver_pref)}, J={J_term}, T2={T2_term}, A={self.A}, C={self.C}, time={self.time_s}"

    def to_lmfit_expr(self) -> str:
        return f"({fmt_pref(self.driver_pref)})*({float(self.A)})*({self._factor_expr()})+({float(self.C)})"

    def validate(self, target_peak, target_pref: ParamRef, driver_peak, driver_pref, slice_states) -> List[str]:
        errors = []
        if driver_peak is None or driver_pref is None:
            errors.append(f"J-modulation constraint on {fmt_pref(target_pref)}: driver peak not found")
        if str(target_pref.name).lower() != "amp" or str(self.driver_pref.name).lower() != "amp":
            errors.append(f"J-modulation constraint on {fmt_pref(target_pref)} must link amplitude to amplitude.")
        if self.time_s is None:
            errors.append(f"J-modulation constraint on {fmt_pref(target_pref)} requires time metadata or time=.../t=...")
        if self.T2_number is not None and self.T2_number <= 0:
            errors.append(f"J-modulation constraint on {fmt_pref(target_pref)} requires T2 > 0.")
        for label, val in (("J", self.J_number), ("T2", self.T2_number), ("A", self.A), ("C", self.C), ("time_s", self.time_s)):
            if val is not None and not np.isfinite(float(val)):
                errors.append(f"J-modulation constraint on {fmt_pref(target_pref)}: {label}={val} is not finite")
        return errors
    ...
    def apply_to_lmfit(self, *, params, registry, allow_external: bool = False, vary: bool = True,
        ...
        if self.J_name:
            ensure_J_param_in_lmfit(params=params, J_name=self.J_name, Jseed_registry=Jseed_registry or Tseed_registry or {})
        if self.T2_name:
            ensure_k_param_in_lmfit(params=params, T_name=self.T2_name, Tseed_registry=Tseed_registry or {})
        ...
        if same_slice and drv_key in params:
            ...
            return
        if not allow_external:
            raise ValueError(f"Cannot apply cross-slice J-modulation on {target_key} when allow_external=False")
        if vary and drv_key in params:
            params[target_key].set(expr=self.to_lmfit_expr(), vary=True)
            return
        ...
        if self.J_name:
            if f"J__{self.J_name}" in params:
                ...
                J_val, _bounds, _fixed = resolve_J_from_registry(J_name=self.J_name, Jseed_registry=Jseed_registry or Tseed_registry or {})
        else:
            J_val = float(self.J_number)
        if self.T2_name:
            k_name = f"k__{self.T2_name}"
            if k_name in params:
                k_val = float(params[k_name].value)
                T2_val = 1.0 / k_val
            else:
                T2_val = resolve_k_from_Tseed_registry(T_name=self.T2_name, Tseed_registry=Tseed_registry or {})[0]
                T2_val = 1.0 / T2_val
        else:
            T2_val = float(self.T2_number)
        val = drv_val * float(self.A) * self._factor_value(J_val, T2_val) + float(self.C)
        params[target_key].set(value=float(val), expr="", vary=False)
...
class FitOrchestrator:
    ...
    Jseed_registry: Mapping[str, Mapping[str, Any]] = field(default_factory=dict)
        ...
        for tgt, rule in rules_internal:
            try:
                ...
                rule.apply_to_lmfit(
                    ...
                    Tseed_registry=Tseed_registry if Tseed_registry is not None else self.Tseed_registry,
                    Jseed_registry=Jseed_registry if Jseed_registry is not None else self.Jseed_registry,
                )
            except Exception as e:
...
register_constraint_rule(ConstraintType.J_COUPLING, JCouplingConstraint)
register_constraint_rule(ConstraintType.J_MODULATION, JModulationConstraint)